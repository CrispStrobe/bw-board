...
| [xv6 x86](https://github.com/mit-pdos/xv6-public) | MIT source builds reproducibly at pinned revision `eeb7b415`; IBM 5170 Rev1 reaches POST memory status codes but remains in its 0x31/0x32 verification loop for 10M instructions with zero ATA commands, so this is not yet an OS acceptance | A generated image can be bundled with the MIT notice; do not check in build products until the boot receipt is green | Audit DMA/PIT/POST handoff against the IBM ROM, then assert protected-mode paging, traps, IDE, and `init` |
