...
| [xv6 x86](https://github.com/mit-pdos/xv6-public) | MIT source builds reproducibly at pinned revision `eeb7b415`; 32-bit ATA PIO and CR4/PSE reach the protected kernel, which visibly panics at `mpinit` without an MP table/LAPIC/IOAPIC; not an OS acceptance | A generated image can be bundled with the MIT notice; do not check in build products until the boot receipt is green | Add an auditable MP/APIC platform path (or a separately labelled UP guest build), then assert paging, traps, IDE, and `init` |
