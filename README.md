...
| [xv6 x86](https://github.com/mit-pdos/xv6-public) | MIT source builds reproducibly at pinned revision `eeb7b415`; 32-bit ATA PIO and CR4/PSE reach protected mode, and the stock image reaches the `xv6...` banner through the WIP MP/LAPIC/IOAPIC path; interrupt-driven userland remains unaccepted | A generated image can be bundled with the MIT notice; do not check in build products until the boot receipt is green | Continue APIC timer/IDE delivery and assert traps, filesystem, and `init` before calling the stock image accepted |
