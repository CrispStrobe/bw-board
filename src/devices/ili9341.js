 ...
 * (§8.2.33).
 *
 * v2 additions (measured from the Adafruit driver differential):
 * - Display inversion: INVOFF (0x20) / INVON (0x21) — §8.2.14/15
 * - Vertical scroll: VSCRDEF (0x33) + VSCRSADD (0x37) — §8.2.24/25
 * - MADCTL BGR bit (bit 3) honored in ili9341Rgba — §8.2.29
 * - RAMRD (0x2E) with dummy-byte rule — §8.2.23, MISO readback
 * - Power/gamma opcodes consumed with named comments, no longer
 *   landing in unknown[]
 */
...
export function registerILI9341() {
    registerDevice('ili9341', {
        ...
        // plus miso for readback.
        terminals: ['vcc', 'gnd', 'cs', 'rst', 'dc', 'mosi', 'sck', 'miso', 'led'],

        init() {
            return {
                drives: { miso: { vTh: 0, rTh: R_OFF } },  // high-Z until read
        ...
        update(part, state, read, tNs) {
            ...
            if (cs) {
                ...
                // Stop MISO output when deselected
            ...
            if (sck && !state._lastSck) { // rising edge: sample MOSI (§7.1.9)
                ...
                if (state._bits === 8) {
                    ...
                    this._byteIn(state, state._byte, dc, vdd);
                    state._bits = 0; state._byte = 0;
                }
            }

            // MISO output for RAMRD: shift out on SCK falling edge (§7.1.9)
        ...
        _byteIn(state, byte, isData, vdd) {
            if (!isData) { // command byte (§8.1)
                ...
                switch (byte) {
                    ...
                    // These are sent by every Adafruit/MCUFRIEND init sequence.
                    // They configure internal power, gamma curves, and panel
                    // timings that have no observable effect in a simulation.
            ...
            if (state._ramReading) return; // data bytes during read are master MOSI traffic, ignored
            ...
            switch (state._cmd) {
                ...
                case 0x2b: // PASET: y0/y1 (§8.2.21)
                    ...
                    break;
                case 0x33: // VSCRDEF — vertical scroll definition (§8.2.24)
                    if (p.length === 6) {
                        state.scrollTFA = (p[0] << 8) | p[1]; // top fixed area
                        state.scrollVSA = (p[2] << 8) | p[3]; // vertical scrolling area
                        state.scrollBFA = (p[4] << 8) | p[5]; // bottom fixed area
                    }
                    break;
                case 0x36: // MADCTL (§8.2.29)
                    state.madctl = p[0];
                    break;
                case 0x37: // VSCRSADD — vertical scroll start address (§8.2.25)
                    if (p.length === 2) {
                ...
                case 0x3a: // COLMOD (§8.2.33): 0x55 = 16-bit, 0x66 = 18-bit
                    ...
                    break;
                default: break; // parameters of power/gamma/vendor commands: consumed
            }
        },

        /** Load the RAMRD read buffer with dummy + pixel bytes at the cursor. */
        _loadReadBuf(state) {
            // §8.2.23: first byte is dummy, then 3 bytes per pixel (R, G, B)
            // from the 565 GRAM. We pre-load enough for the current pixel.
            ...
            // Expand 565 to 8-bit R, G, B
            const r = ((px >> 11) & 0x1f) << 3;
            const g = ((px >> 5) & 0x3f) << 2;
            const b = (px & 0x1f) << 3;
            // Dummy byte (8 bits of nothing) + R + G + B
...
export function ili9341Rgba(state) {
    ...
    // Only apply scroll when VSCRDEF has been set (VSA < full height)
    const scrollActive = (tfa + vsa + (state.scrollBFA || 0)) === ILI9341_H
        && vsa < ILI9341_H && ss !== 0;

    for (let row = 0; row < ILI9341_H; row++) {
        // Vertical scroll: rows in [TFA, TFA+VSA) are offset by scrollStart
        let srcRow = row;
