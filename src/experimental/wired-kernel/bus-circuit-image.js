...
export function prepareBusCircuit({circuit,image,bus,phase,banks}) {
    ...
    return {
        ...
        initialize({e,p,put,inspect,inspectMemory,phaseMethods,memoryFault,stageAttribution=false}) {
            ...
            const submit=transaction=>{
                ...
                if(!KINDS.includes(kind)||locked!==false||!Number.isSafeInteger(address)||![1,2].includes(width)||!Number.isInteger(value))
                    throw new CircuitFault('UNSUPPORTED_TRANSACTION','memory subset');
                count('wasmBusSubmitEntries');const code=e.bus_submit(KINDS.indexOf(kind),address,width,value,0);
            ...
            const inspectJSStageAttribution=()=>({...jsStage});
            const resetJSStageAttribution=()=>{for(const name of Object.keys(jsStage))jsStage[name]=0;};
            return {beginClock,endClock,submit,runUntilCompletion,inspectBus,inspectLifecycle:lifecycle,
                ...
                inspectJSStageAttribution,resetJSStageAttribution};
        }
    };
}
