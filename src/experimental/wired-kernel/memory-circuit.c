 ...
 * settles actually entered, and post-memory settles skipped because no output
 * driver changed. */
...
u32 memory_pass_counters_version(void){return 2;}
...
u32 settle_memory_circuit(const u32 *c,u32 passes,u32 *fault) {
    ...
    for(u32 pass=0;pass<passes;pass++) {
        ...
        u32 changed=0,driver_changed=0;
        for(u32 b=0;b<banks;b++) {
            ...
            if(U8(26)[b])for(u32 bit=0;bit<8;bit++){
                u32 id=output_ids[b*8+bit],value=U8(25)[b*8+bit];
                if(U8(4)[id]!=value)driver_changed=1;
                if(write_owned_driver_tagged(c,id,value,PRODUCER_MEMORY_BANK)){fault[0]=1;fault[1]=2;return 1;}
            }
        }
        if(driver_changed) {
            memory_pass_work[6]++;result=settle_context(c);
            if(result&0x80000000u){fault[0]=1;fault[1]=result&0x7fffffffu;return 1;}
        } else memory_pass_work[7]++;
