    ...
    return {
        ...
        initialize({e,p,put,inspect,fault}) {
            ...
            const compileSchedule=steps=>{
                ...
                const offsets=new Uint32Array(steps.length+1),flags=new Uint8Array(steps.length),expected=new Uint32Array(steps.length),drivers=[],values=[];
                steps.forEach((step,i)=>{
                    ...
                    for(const [pin,value] of Object.entries(step.values)) {
                        ...
                        drivers.push(ids.get(pin));values.push(code);
                        if(drivers.length>maxUpdates)throw new RangeError('schedule update capacity');
                    }
                ...
                const handle=Object.freeze({periods:steps.length,updates:drivers.length});
            ...
            const runSchedule=handle=>{
                ...
                const result=e.run_latched_memory_schedule(p.phaseContext,handle.periods,handle.updates,p.scheduleOffsets,p.scheduleIds,p.scheduleValues,
