...
/* Explicit writer seam: frontier implementations must route this through their
 * authoritative write_owned_driver path, not assume a later whole-driver scan. */
static void stage_bus_driver(const u32 *c,u32 id,u8 value){((u8*)(unsigned long)c[4])[id]=value;}
...
u32 begin_bus_memory_clock(const u32*p,u32*fault) {
    ...
    for(u32 i=0;i<p[6];i++)stage_bus_driver(W(0),W(4)[i],B(5)[i]);
    ...
    for(u32 i=0;i<48;i++)stage_bus_driver(W(0),W(3)[i],(u8)output[i]);
    if((result=settle(p,fault)))return result;
