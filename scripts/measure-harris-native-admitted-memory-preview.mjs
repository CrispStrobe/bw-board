/** Same-runner A/B measurement of admitted native memory preview validation. */
...
export const CANDIDATE_REVISION='d3f8227cdf424dde7a66a4055099cf7ad68d86ac';
...
export function assertAdmittedPreviewReconciliation(candidate,base,label='sample'){
    ...
    for(const name of legacyMemory)
    ...
    assert.equal(after.ownedPreviewCalls,after.previewCalls,`${label}: every admitted preview uses the owned entry`);
    assert.equal(after.checkedValidationBanks,0,`${label}: admitted preview executes no generic bank validation`);
    assert.equal(after.checkedValidationPinRecords,0,`${label}: admitted preview executes no generic pin validation`);
    assert.ok(before.previewBanks>0,`${label}: measured base exercised memory previews`);
    return before.previewBanks*28;
}
...
async function main(){
    ...
    if(options.experimental!==true||!options['base-dir']||!options['candidate-dir']||!options['base-wasm']||!options['candidate-wasm'])
        throw new Error('usage: node --expose-gc scripts/measure-harris-native-admitted-memory-preview.mjs --experimental --base-dir=DIR --candidate-dir=DIR --base-wasm=FILE --candidate-wasm=FILE');
    ...
    for(const group of interleavedOrder(settings.warmupRounds,settings.rounds)){
        ...
        const validationPinRecordReduction=assertAdmittedPreviewReconciliation(pair.candidate.semantic,pair.base.semantic,`${group.phase} ${group.round}`);
        const record={phase:group.phase,round:group.round,order:group.order,validationPinRecordReduction,
    ...
    const validationPinRecordReduction=paired[0].validationPinRecordReduction;
    for(const pair of paired)assert.equal(pair.validationPinRecordReduction,validationPinRecordReduction,
        'deterministic generic validation reduction');
    ...
    console.log(JSON.stringify({schemaVersion:1,workload:'owned-harris-store-loop-memory-only-admitted-incremental-native-admitted-memory-preview',
        ...
        expectedByVariant,validationPinRecordReduction,pairedActiveThroughputRatio,positionEffects,metrics,warmups,samples,paired,
        limitations:['Same hosted job, process and balanced AB/BA order reduce runner drift but do not make shared-host timing deterministic.',
            ...
            'The candidate bypasses only repeated generic buffer-shape validation after the private circuit context and graph were admitted; public and checked preview calls remain fully validated.',
            'Every semantic preview, all-bank preflight-before-commit ordering, memory commit, output publication and required settle still execute.',
