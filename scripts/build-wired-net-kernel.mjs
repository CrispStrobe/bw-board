...
const args=['--target=wasm32','-O3','-nostdlib','-fno-builtin','-Werror','-Wall','-Wextra',
    ...
    '-Wl,--no-entry','-Wl,--export=arena_ptr','-Wl,--export=arena_capacity','-Wl,--export=resolve_nets','-Wl,--export=settle_owned','-Wl,--export=owned_kernel_version','-Wl,--export-memory',source,'-o',output];
const version=execFileSync(clang,['--version'],{encoding:'utf8'}).trim();
execFileSync(clang,args,{stdio:'inherit'});
const hash=bytes=>createHash('sha256').update(bytes).digest('hex');
const report={prototype:'wired-owned-combinational-only',compiler:version,args,
