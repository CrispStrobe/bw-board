...
function runPCjs(){const c=new CPU({id:"task386.cpu",model:80386}),b=new QuietBus({id:"task386.bus",busWidth:32},c);if(!b.addMemory(0,0x10000,Memory.TYPE.RAM))throw new Error("PCjs memory allocation failed");c.bus=b;install((a,v)=>b.setByteDirect(a,v));c.setCS(0);c.setIP(0);c.setDS(0);c.setES(0);c.setSS(0);c.setSP(0x800);c.setPS(2);for(let i=0;i<limit&&!(c.intFlags&X86.INTFLAG.HALT);i++)c.stepCPU(0);return result(c,a=>b.getByteDirect(a),false);}
