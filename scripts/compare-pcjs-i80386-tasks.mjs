...
function install(w){
  ...
  desc(w,0x208,0x1000,0xfffff,0x9a);desc(w,0x210,0,0xfffff,0x92);
  desc(w,0x218,0x400,0x67,0x89,0);desc(w,0x220,0x500,0x67,0x89,0);
  put(w,0x1000,[0xb8,0x18,0,0x0f,0,0xd8,0xb8,0,0x70,0,0,0x0f,0x22,0xd8,
    0xb8,1,0,0,0x80,0x0f,0x22,0xc0,0x9a,0,0,0,0,0x20,0,0xf4]);
  put(w,0x1100,[0xa1,0,0x80,0,0,0xa3,0,6,0,0,0xcf]);
...
function runPCjs(){const c=new CPU({id:"task386.cpu",model:80386}),b=new QuietBus({id:"task386.bus",busWidth:32},c);if(!b.addMemory(0,0x10000,Memory.TYPE.RAM))throw new Error("PCjs memory allocation failed");c.bus=b;install((a,v)=>b.setByteDirect(a,v));c.setCS(0);c.setIP(0);c.setDS(0);c.setES(0);c.setSS(0);c.setSP(0x800);c.setPS(2);for(let i=0;i<limit&&!(c.intFlags&X86.INTFLAG.HALT);i++){try{c.stepCPU(0);}catch(error){throw new Error(`PCjs abort at step ${i} ${c.getCS().toString(16)}:${c.getIP().toString(16)} CR0=${(c.regCR0>>>0).toString(16)} CR3=${(c.regCR3>>>0).toString(16)} raw=${String(error)}`);}}return result(c,a=>b.getByteDirect(a),false);}
const reference=runPCjs(),actual=runLocal();if(mutation==="result")actual.result^=1;
const expected={halted:true,cs:8,eip:0x1020,tr:0x18,cr3:0x7000,result:0x22222222,oldBusy:11,newBusy:9,backlink:0x18};
