...
for(let i=0;i<args.length;i+=2) {
    if(!['--max-clocks','--pic-mode','--ram-kib'].includes(args[i])||!args[i+1]||options.has(args[i])) {
        console.error('Usage: probe-harris-bios.mjs [--max-clocks N] [--pic-mode legacy-buffered|single-unbuffered] [--ram-kib 64..640]');process.exit(1);
    }
...
if(!['legacy-buffered','single-unbuffered'].includes(picMode)||!/^\d+$/.test(rawRAM)||ramKiB<64||ramKiB>640||ramKiB%64) {
...
const board=createHarrisMemoryBoard({enabled:true,rom,romLowAlias:true,ramBytes:ramKiB*1024,textRAM:true,
    intrEnabled:true,ioEnabled:true,interruptDevice:pic,timerDevice:timer});
...
console.log(JSON.stringify({accepted:false,diagnosticOnly:true,maxClocks,completedClocks:clocks,outcome,
    configuration:{ramBytes:ramKiB*1024,textRAM:true,romLowAlias:true,picMode,timerClockHalfPeriod:8},
    romSHA256:hash(rom),sourceHashes,
    cs:cpu.cs,ip:cpu.ip,retired:cpu.retired,registers:cpu.inspect().registers,pic:pic.inspect(),timer:timer.inspect(),
