...
const sourcePaths=[
    ...
    'scripts/lib/at-dos-acceptance.mjs','scripts/run-i80386-at-dos-acceptance.mjs'];
...
try {
    ...
    throw new Error('386 AT DOS run refused: an executed source path differs from HEAD');
}
...
const machineProfile=PCAT80386_EXPERIMENTAL;
...
let machine;
machine=new ExperimentalI80386ATMachine(machineProfile,{onPortAccess:event=>{
...
if(reset.cs!==0xf000||reset.ip!==0xfff0||reset.pc!==0xfffffff0)
    throw new Error(`80386 reset entry mismatch: ${JSON.stringify(reset)}`);

let steps=0;
machine.step();
const firstFetchTrace=[reset.firstByte,reset.pc];
steps=1;
...
const report={schema:'astra.i80386-at-dos-acceptance.v1',passed,stepLimit,steps,
    scope:fullBootAccepted?'genuine-reset IBM 5170 Rev1 BIOS on the experimental 386 adapter, FDC/DMA DOS boot and keyboard shell command':
        'bounded genuine-reset IBM 5170 Rev1 POST progression on the experimental 386 adapter',
...
for(const [file,before] of Object.entries(sourceSha256)) {
    ...
    if(after!==before)throw new Error(`386 AT DOS run refused: executed source changed during execution: ${file}`);
}
if(execFileSync('git',['rev-parse','HEAD'],{cwd:root,encoding:'utf8'}).trim()!==executionRevision)
    throw new Error('386 AT DOS run refused: HEAD changed during execution');
