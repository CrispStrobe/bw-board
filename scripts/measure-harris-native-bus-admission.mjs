/** Same-runner A/B measurement of instance-local immutable bus-map admission. */
...
export const CANDIDATE_REVISION='9550bfb49c23ae770e188569fe52476deb8d22ff';
...
export function assertBusAdmissionReconciliation(candidate,base,label='sample'){
    ...
    assert.equal(candidate.producerWork.producers.busExternal.attempts,candidate.periods*10,
        `${label}: ten live external values remain staged and validated per period`);
    assert.equal(base.busAdmissionAtConstruction,null,`${label}: historical base has no bus-admission ABI`);
    assert.equal(base.busAdmissionAfterInitialize,null);assert.equal(base.busAdmissionAfterRun,null);
    assert.deepEqual(candidate.busAdmissionAtConstruction,{attempts:1,admissions:1,failures:0,
        inputMapVisits:24,outputMapVisits:48,externalMapVisits:10},`${label}: exact immutable admission`);
    const zero={attempts:0,admissions:0,failures:0,inputMapVisits:0,outputMapVisits:0,externalMapVisits:0};
    assert.deepEqual(candidate.busAdmissionAfterInitialize,zero,`${label}: reset and hardware initialization preserve grant`);
    assert.deepEqual(candidate.busAdmissionAfterRun,zero,`${label}: runtime repeats no immutable admission work`);
    return Object.freeze({immutableMapComparisonsRemoved:candidate.periods*82,
        liveLevelComparisonsRetained:candidate.producerWork.producers.busExternal.attempts});
}
...
async function sample(variant,{iterations,batchPeriods,wallBudgetMS}){
    ...
    const busAdmissionAtConstruction=board.inspectBusAdmission?.()??null;board.resetBusAdmissionCounters?.();
    cpu.initialize();const busAdmissionAfterInitialize=board.inspectBusAdmission?.()??null;
    ...
    const busAdmissionAfterRun=board.inspectBusAdmission?.()??null;
    ...
    return {label:variant.name,activeMS:result.activeMS,wallMS,activePeriodsPerSecond:result.periods*1000/result.activeMS,
        wallPeriodsPerSecond:result.periods*1000/wallMS,semantic:{stateHash:state.stateHash,componentHashes:state.componentHashes,
            ...
            work,producerWork,busAdmissionAtConstruction,busAdmissionAfterInitialize,busAdmissionAfterRun}};
}
async function main(){
    parseOptions(process.argv.slice(2));
    if(options.experimental!==true||!options['base-dir']||!options['candidate-dir']||!options['base-wasm']||!options['candidate-wasm'])
        throw new Error('usage: node --expose-gc scripts/measure-harris-native-bus-admission.mjs --experimental --base-dir=DIR --candidate-dir=DIR --base-wasm=FILE --candidate-wasm=FILE');
    ...
    for(const group of interleavedOrder(settings.warmupRounds,settings.rounds)){
        ...
        const structural=assertBusAdmissionReconciliation(pair.candidate.semantic,pair.base.semantic,`${group.phase} ${group.round}`);
    ...
    const {immutableMapComparisonsRemoved,liveLevelComparisonsRetained}=paired[0];
    for(const pair of paired)assert.deepEqual({immutableMapComparisonsRemoved:pair.immutableMapComparisonsRemoved,
        liveLevelComparisonsRetained:pair.liveLevelComparisonsRetained},
    {immutableMapComparisonsRemoved,liveLevelComparisonsRetained},'deterministic structural work');
    ...
    console.log(JSON.stringify({schemaVersion:1,workload:'owned-harris-store-loop-memory-only-admitted-incremental-bus-map-admission',
        ...
        expectedByVariant,immutableMapComparisonsRemoved,liveLevelComparisonsRetained,
        ...
        limitations:['Same hosted job, process and balanced AB/BA order reduce runner drift but do not make shared-host timing deterministic.',
            ...
            'The candidate captures immutable bus maps only for the explicit admitted path; raw callers retain full per-call map validation.',
            'Every mutable external 0/1/X/Z level is still validated before the first driver write on every admitted period.',
            'Phase validation, bus sequencing, scheduling, callbacks, debugging and replay-visible state are unchanged.',
