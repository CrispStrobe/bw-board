...
function bootSector() {
  ...
  emit(0x31, 0xc0, 0x8e, 0xd8, 0x8e, 0xc0, 0x8e, 0xd0, 0xbc, 0x00, 0x7c,
    0xfb, 0xfc); // STI; CLD.
  const zeroBuffer = () => emit(0x31, 0xc0, 0xbf, 0x00, 0x06, 0xb9, 0x00, 0x01,
    0xf3, 0xab); // Zero all 512 bytes at 0000:0600.
  const poisonBuffer = () => emit(0xb8, 0xa5, 0xa5, 0xbf, 0x00, 0x06,
    0xb9, 0x00, 0x01, 0xf3, 0xab); // Detect an incomplete BIOS readback.
  zeroBuffer();
  ...
  emit(0x75, 0);
  emit(0xbe, 0x10, 0x06, 0xb9, 0xf0, 0x01); // Check the remaining 496 bytes.
