...
function bootSector() {
  ...
  emit(0x31, 0xc0, 0x8e, 0xd8, 0x8e, 0xc0, 0x8e, 0xd0, 0xbc, 0x00, 0x7c, 0xfb);
  const patternAddressPatch = code.length + 1;
  emit(0xbe, 0, 0, 0xbf, 0x00, 0x06, 0xb9, 0x08, 0x00, 0xf3, 0xa5);
  const transfer = operation => emit(0xb8, 0x01, operation, 0xbb, 0x00, 0x06,
    0xb9, 0x51, 0x31, 0xba, 0x80, 0x03, 0xcd, 0x13);
  transfer(0x03);
  const writeFailure = code.length + 1;
  emit(0x72, 0, 0x31, 0xc0, 0xbf, 0x00, 0x06, 0xb9, 0x08, 0x00, 0xf3, 0xab);
  transfer(0x02);
  ...
  emit(0x75, 0, 0xb0, 0xa5, 0xe6, 0x80, 0xfa, 0xf4, 0xeb, 0xfd);
  ...
  for (const patch of [writeFailure, readFailure, compareFailure])
