...
export async function runNativePhaseScheduleOracle({wasmBytes,yieldTask=()=>Promise.resolve(),stopped=()=>false,admittedGraph=false,incrementalGraph=false}={}) {
    const f=await createPhaseCircuitOracle({wasmBytes,schedule:true,admittedGraph,incrementalGraph}),reference=await createPhaseCircuitOracle({wasmBytes});
