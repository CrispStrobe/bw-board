/** Same-runner A/B measurement of combined sparse bus-output and admitted memory-preview hot paths. */
...
export const CANDIDATE_REVISION='f62b1761bbf3502dc3905e0ee333d8cd9c635b07';
...
export function assertCombinedReconciliation(candidate,base,label='sample'){
    ...
    const legacyMemory=['changedBanks','passes','postMemorySettles','postMemorySettlesWithoutDriverChange',
        'presentBanks','previewBanks','previewCalls','settleCalls'];
    assert.deepEqual(Object.keys(base.producerWork.memory).sort(),[...legacyMemory].sort(),`${label}: base memory schema`);
    assert.deepEqual(Object.keys(candidate.producerWork.memory).sort(),[...legacyMemory,'ownedPreviewCalls',
        'checkedValidationBanks','checkedValidationPinRecords'].sort(),`${label}: candidate memory schema`);
    for(const name of legacyMemory)assert.equal(candidate.producerWork.memory[name],base.producerWork.memory[name],
        `${label}: memory ${name}`);
    ...
    const memory=candidate.producerWork.memory;
    assert.equal(memory.ownedPreviewCalls,memory.previewCalls,`${label}: every admitted preview uses the owned entry`);
    assert.equal(memory.checkedValidationBanks,0,`${label}: admitted preview executes no generic bank validation`);
    assert.equal(memory.checkedValidationPinRecords,0,`${label}: admitted preview executes no generic pin validation`);
    return Object.freeze({comparisonReduction:(base.work.driverComparisons-candidate.work.driverComparisons)/base.work.driverComparisons,
        skippedBusOutputAttempts:bb.attempts-cb.attempts,validationPinRecordReduction:base.producerWork.memory.previewBanks*28});
}
...
async function main(){
    ...
    if(options.experimental!==true||!options['base-dir']||!options['candidate-dir']||!options['base-wasm']||!options['candidate-wasm'])
        throw new Error('usage: node --expose-gc scripts/measure-harris-native-hotpath-combined.mjs --experimental --base-dir=DIR --candidate-dir=DIR --base-wasm=FILE --candidate-wasm=FILE');
    ...
    for(const group of interleavedOrder(settings.warmupRounds,settings.rounds)){
        ...
        const structural=assertCombinedReconciliation(pair.candidate.semantic,pair.base.semantic,`${group.phase} ${group.round}`);
    ...
    const {comparisonReduction,skippedBusOutputAttempts,validationPinRecordReduction}=paired[0];
    for(const pair of paired)assert.deepEqual({comparisonReduction:pair.comparisonReduction,
        skippedBusOutputAttempts:pair.skippedBusOutputAttempts,validationPinRecordReduction:pair.validationPinRecordReduction},
    {comparisonReduction,skippedBusOutputAttempts,validationPinRecordReduction},'deterministic structural reductions');
    ...
    console.log(JSON.stringify({schemaVersion:1,workload:'owned-harris-store-loop-memory-only-admitted-incremental-native-hotpath-combined',
        ...
        expectedByVariant,comparisonReduction,skippedBusOutputAttempts,validationPinRecordReduction,
        pairedActiveThroughputRatio,positionEffects,metrics,warmups,samples,paired,
        limitations:['Same hosted job, process and balanced AB/BA order reduce runner drift but do not make shared-host timing deterministic.',
            ...
            'The candidate combines only sparse native CPU bus-output publication and admitted memory-preview validation; neither variant changes execution defaults.',
            'The mask compares raw per-driver 0/1/X/Z codes. Other drivers remain responsible for contention and resolved-net changes.',
            'The memory fast path bypasses only repeated generic buffer-shape validation after admission; checked/public calls and all semantic preview and peer-atomic commit work remain.',
