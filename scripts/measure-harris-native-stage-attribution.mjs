...
export function rotatedOrder(round){const labels=['master','diagnosticOff','counterOn'],start=(round-1)%3;return [...labels.slice(start),...labels.slice(0,start)];}
export function controlGateResult(ratios){const observations=Object.fromEntries(Object.entries(ratios).map(([name,value])=>
...
const options={};
function parseOptions(args){for(const arg of args){const m=/^--(experimental|profile-only|classify-profile=(.+)|profile-receipt=(.+)|master-dir=(.+)|candidate-dir=(.+)|master-wasm=(.+)|off-wasm=(.+)|counter-wasm=(.+)|profile-wasm=(.+)|candidate-revision=([0-9a-f]{40})|iterations=([0-9]+)|warmup-rounds=([0-9]+)|rounds=([0-9]+)|profile-repetitions=([0-9]+))$/.exec(arg);
...
async function loadVariant(name,directory,wasmPath,revision,stageAttribution,stageProfileNames=false){
    ...
    assert.equal(build.wasmSHA256,hash(wasm),`${name}: wasm receipt`);assert.equal(build.stageAttribution??false,stageAttribution,`${name}: counter build mode`);
    ...
    return {name,directory,revision,wasm,HarrisBootCPU:cpu.HarrisBootCPU,createROM:rom.createHarrisStoreLoopROM,createBoard:board.createHarrisNativeMemoryBoard,run:run.runHarrisTransactions,stageAttribution,
        provenance:{revision,wasmSHA256:build.wasmSHA256,manifestSHA256:hash(manifestBytes),stageAttribution,stageProfileNames,compiler:build.compiler,args:build.args,sourceHashes:build.sourceHashes,
...
async function main(){parseOptions(process.argv.slice(2));if(options['classify-profile']){const profile=JSON.parse(readFileSync(options['classify-profile'])),receipt=JSON.parse(readFileSync(options['profile-receipt']));const classification=classifyProfile(profile);assert.ok(receipt.samples?.length,'profile receipt');console.log(JSON.stringify({schemaVersion:1,profileReceipt:receipt,profileGates:PROFILE_GATES,classification,limitations:['Leaf samples are assigned once; inclusive stacks are not summed. Idle and garbage-collector samples are disclosed and excluded from the classification denominator.','Runtime, harness and fixture-construction samples contribute to overall classification only; they never contribute to actionableSamples or actionableRatio.','The named-profile build intentionally uses noinline stage boundaries and the CPU profile covers the whole Node process; profile shares are diagnostic and do not measure production timing.']},null,2));return;}
    ...
    const master=await loadVariant('master',options['master-dir'],options['master-wasm'],MASTER_REVISION,false),off=await loadVariant('diagnosticOff',options['candidate-dir'],options['off-wasm'],candidate,false),counter=await loadVariant('counterOn',options['candidate-dir'],options['counter-wasm'],candidate,true,false),profile=await loadVariant('namedProfile',options['candidate-dir'],options['profile-wasm'],candidate,false,true);
    ...
    const warmupRounds=integer('warmup-rounds',3,9),rounds=integer('rounds',12,30),variants={master,diagnosticOff:off,counterOn:counter},warmups=[],samples=[];let expected=null,sequence=0;
    ...
    const ratios={candidateOffToMaster:metrics.diagnosticOff.median/metrics.master.median,counterOnToOff:metrics.counterOn.median/metrics.diagnosticOff.median};
    ...
    const controlGate=controlGateResult(ratios),report={schemaVersion:1,accepted:controlGate.passed,controlGate,workload:'owned-harris-store-loop-memory-only-admitted-incremental-native-stage-attribution',measurementRevision:candidate,clean:true,settings:{iterations,warmupRounds,rounds,batchPeriods:8192,wallBudgetMS:1000},host:{hostname:hostname(),platform:platform(),arch:arch(),cpu:cpus()[0]?.model,node:process.version,loadavg:loadavg()},variants:Object.fromEntries(Object.entries(variants).map(([k,v])=>[k,v.provenance])),expected,metrics,ratios,warmups,samples,limitations:['Diagnostic counters wrap modulo 2^32 and preserve all existing counter layouts and meanings.','Master, diagnostics-off candidate, and counter-on candidate run in rotating same-job order; shared-host timing remains nondeterministic.','The counter-on timing build does not use noinline profile boundaries. CPU profiling uses a distinct named-profile build in separate whole-process runs.','Memory-only workload; no DOS, peripherals, browser capacity, optimization, or default-change claim.']};
