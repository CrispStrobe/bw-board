...
export function rotatedOrder(round){const labels=['master','diagnosticOff','nativeCounter','combinedCounter'],start=(round-1)%4;return [...labels.slice(start),...labels.slice(0,start)];}
...
async function loadVariant(name,directory,wasmPath,revision,buildStageAttribution,stageProfileNames=false,jsStageAttribution=buildStageAttribution){
...
async function main(){parseOptions(process.argv.slice(2));if(options['classify-profile']){const profile=JSON.parse(readFileSync(options['classify-profile'])),receipt=JSON.parse(readFileSync(options['profile-receipt']));const classification=classifyProfile(profile);assert.ok(receipt.samples?.length,'profile receipt');console.log(JSON.stringify({schemaVersion:1,profileReceipt:receipt,profileGates:PROFILE_GATES,classification,limitations:['Leaf samples are assigned once; inclusive stacks are not summed. Idle and garbage-collector samples are disclosed and excluded from the classification denominator.','Whole-process URL buckets can include board construction and final inspection frames from CPU, board, and wrapper modules; actionable shares are diagnostic localization rather than active-interval production timing.','The named-profile build intentionally uses noinline stage boundaries and the CPU profile covers the whole Node process; profile shares are diagnostic and do not measure production timing.']},null,2));return;}
    ...
    for(let round=1;round<=warmupRounds+rounds;round++){const phase=round<=warmupRounds?'warmup':'measured',ordinal=phase==='warmup'?round:round-warmupRounds;for(let position=0;position<4;position++){const label=rotatedOrder(round)[position],value=await sample(variants[label],iterations);expected??=value.semantic;assertSemantic(value.semantic,expected,`${phase} ${ordinal} ${label}`);(phase==='warmup'?warmups:samples).push({sequence:++sequence,phase,round:ordinal,position:position+1,...value});}}
    ...
    const ratios={candidateOffToMaster:metrics.diagnosticOff.median/metrics.master.median,nativeCounterToOff:metrics.nativeCounter.median/metrics.diagnosticOff.median,
        combinedCounterToNative:metrics.combinedCounter.median/metrics.nativeCounter.median,combinedCounterToOff:metrics.combinedCounter.median/metrics.diagnosticOff.median};
    ...
    const controlGate=controlGateResult({candidateOffToMaster:ratios.candidateOffToMaster,combinedCounterToOff:ratios.combinedCounterToOff}),report={schemaVersion:1,accepted:controlGate.passed,controlGate,workload:'owned-harris-store-loop-memory-only-admitted-incremental-native-stage-attribution',measurementRevision:candidate,clean:true,settings:{iterations,warmupRounds,rounds,batchPeriods:8192,wallBudgetMS:1000},host:{hostname:hostname(),platform:platform(),arch:arch(),cpu:cpus()[0]?.model,node:process.version,loadavg:loadavg()},variants:Object.fromEntries(Object.entries(variants).map(([k,v])=>[k,v.provenance])),expected,metrics,ratios,warmups,samples,limitations:['Diagnostic counters wrap modulo 2^32 and preserve all existing counter layouts and meanings.','JavaScript crossing counters include accepted submit calls, returned run calls, and native run faults carrying progress; calls rejected before the corresponding Wasm boundary are deliberately excluded.','Master, diagnostics-off, native-counter, and combined-counter arms run in rotating same-job order; shared-host timing remains nondeterministic.','Native-counter and combined-counter load the exact same Wasm bytes; their ratio isolates the JavaScript stage-counting path. The unchanged acceptance gate applies to combined-counter versus diagnostics-off.','The counter timing build does not use noinline profile boundaries. CPU profiling uses a distinct named-profile build in separate whole-process runs.','Memory-only workload; no DOS, peripherals, browser capacity, optimization, or default-change claim.']};
