...
export function assertStageReceipt(stage,semantic){
    ...
    assert.equal(j.nativeReceiptBytesRead,j.completionObjects*36);
}
...
function stateOf(cpu,board){const bus=board.inspectBus();return {stateHash:hash(JSON.stringify({cpu:cpu.inspect(),bus,memory:['rom0','rom1','ram0','ram1'].map(id=>{const x=board.inspectMemory(id);return {id,bytes:[...x.bytes],writes:x.writes};})})),physicalClock:bus.clock,retired:cpu.retired,writes:[board.inspectMemory('ram0').writes,board.inspectMemory('ram1').writes]};}
async function sample(v,iterations=4096){const board=await v.createBoard({enabled:true,rom:v.createROM(iterations),romLowAlias:true,wasmBytes:v.wasm,admittedGraph:true,incrementalGraph:true,
    ...
    const semantic={stateHash:state.stateHash,periods:result.periods,retired:state.retired,writes:state.writes,physicalClock:state.physicalClock,chunks:result.chunks,yields:0,work,producerWork};
