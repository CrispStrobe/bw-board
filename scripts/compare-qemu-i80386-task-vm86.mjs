...
const build=mkdtempSync(join(tmpdir(),'astra-vmtask-oracle-'));
process.once('exit',()=>rmSync(build,{recursive:true,force:true}));
const object=join(build,'guest.o'),image=join(build,'guest.bin'),debug=join(build,'debug.out');
execFileSync('as',['--32','-o',object,resolve(repo,source)]);
execFileSync('ld',['-m','elf_i386','-Ttext','0x7c00','--oformat','binary','-o',image,object]);
writeFileSync(debug,'');
const run=spawnSync(qemu,['-L',resolve(qemuRoot,'usr/share/seabios'),'-bios','bios.bin','-machine','pc,accel=tcg','-cpu','486','-m','16M','-nic','none','-vga','none','-device','isa-debug-exit,iobase=0xf4,iosize=0x04','-drive',`file=${image},format=raw,if=floppy`,'-display','none','-monitor','none','-serial','none','-debugcon',`file:${debug}`,'-no-reboot'],{env,timeout:5000});
...
const packages={
 'qemu-system-x86_1%3a8.2.2+ds-0ubuntu1.18_amd64.deb':'14602e262627adac030329d2cecfee5f6c0938b34566ff2ea2d4c9a9eb02430d',
 'qemu-system-common_1%3a8.2.2+ds-0ubuntu1.18_amd64.deb':'0a0b744f31e87d72dd0465436b8dfbb1ef9f574ecd0d5a2e9dd8c5cb7d0fef1f',
 'qemu-system-data_1%3a8.2.2+ds-0ubuntu1.18_all.deb':'a14b88d864859bd61c8a3274971da4ecb7da6cec15be6c265d0d411f783d5f2e',
...
const packageRoot=process.env.QEMU_PACKAGES_ROOT;if(!packageRoot)throw new Error('Set QEMU_PACKAGES_ROOT to the downloaded QEMU package directory');
for(const [name,expectedHash]of Object.entries(packages))if(hash(readFileSync(resolve(packageRoot,name)))!==expectedHash)throw new Error(`package hash mismatch: ${name}`);
const qemuExecutableHash=hash(readFileSync(qemu));if(qemuExecutableHash!=='28fa14f1c45fca7422e3ec5737768b33b705de2f31014a76e658d4263464a6a5')throw new Error('unexpected QEMU executable hash');
const biosHash=hash(readFileSync(resolve(qemuRoot,'usr/share/seabios/bios.bin')));
const assemblerVersion=execFileSync('as',['--version'],{encoding:'utf8'}).split('\n')[0],linkerVersion=execFileSync('ld',['--version'],{encoding:'utf8'}).split('\n')[0];
if(biosHash!=='4d597f68e06a0e28498e12e96e1f2ce64aee88a519685c61da92eb775c95a445'||assemblerVersion!=='GNU assembler (GNU Binutils for Ubuntu) 2.42'||linkerVersion!=='GNU ld (GNU Binutils for Ubuntu) 2.42')throw new Error('unexpected BIOS or compiler input');
...
const report={oracle:'QEMU TCG later-model software CPU',revision,qemuVersion,qemuExecutableHash,biosHash,assemblerVersion,linkerVersion,packages,sourceHashes,imageHash:hash(binary),execution:{reference:'BIOS boot and floppy sector load',actual:'same image at 0x7c00; starts owned setup at 0x7e00 after disk load'},mutation,status:differences.length?'fail':'pass',expected,reference,actual,actualState,differences};rmSync(build,{recursive:true,force:true});console.log(JSON.stringify(report,null,2));process.exitCode=differences.length?1:0;
