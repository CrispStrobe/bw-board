...
| --- | --- | --- | --- |
| waveform transient accuracy and breakpoint control | `/root/zenodo_access_diagnosis` (Codex Sol) | 2026-09-14, `lane/waveform-accuracy-sol`, exact base `3e5478b` | **ACTIVE.** Owns coherent numerical diagnosis/correction for retained Si7li rows 158 (1 ns PULSE edge into 1 us RC), 271 (500 kHz SINE RC), and 5434 (purely resistive high-gm VCCS), including endpoint breakpoints, adaptive local-error control and conditioning where the measured cause warrants it. Preserves uniform positive-out raw currents and positive-into `operatingPoint()` results. No comparator tolerance widening, global GMIN retune, source approximation, paused CUI battery lane, or unrelated waveform acceptance edits. Focused self-authored fixtures and independent corpus comparison required before closeout. |
| broad source-waveform transient semantics | `/root/zenodo_access_diagnosis` (Codex Sol), worktree `/mnt/volume1/code/wt/bw-board-waveforms-sol` | `lane/waveform-transient-sol` at exact base `e11fcbd8600e73581b885f10c6e4aeebad35a385` | **ACTIVE 2026-09-14.** Owns exact PWL/EXP and tractable PULSE/SINE completion for voltage/current sources, explicit transient-time value versus DC-bias initialization metadata, source breakpoint scheduling, focused numerical/refusal tests and bounded API/docs updates. No ASC/ASY importer/exporter, semiconductor equation, startup/IC/nodeset approximation, or restricted-source code/payload. |
