...
| --- | --- | --- | --- | --- | --- |
| Level-1 NMOS exact cutoff with one-sided C1 threshold transition | `/root` (Codex bwcx) | `/mnt/volume1/code/wt/bwb-nmos-threshold-cutoff`, branch `lane/nmos-threshold-cutoff` | `src/mna.js`, `test/nmos-operating-point.test.mjs`, and this row only. The exact 1,670-row ADI-v5 model-default replay leaves 174 numerical failures; a representative nominal cutoff has VGS=VTO yet the current symmetric blend manufactures 156.25 nA (`K=0.1`, effective overdrive 1.25 mV), yielding 45.2 uV at source and 0.301 mV at drain while ngspice Level-1 and the hard square law are zero. Replace only the no-`ksubthres` threshold blend with a one-sided C1 transition that is exactly zero at/below threshold, joins the exact square law with value/slope continuity at the existing 5 mV boundary, and leaves declared VDMOS soft-plus unchanged. Prove cutoff zero/current/KCL, active-law continuity, the historical cross-coupled-latch convergence reason, and isolated zero-at-threshold/slope/VDMOS mutations; replay all 174 red rows downstream. No CUI/corpus/pin, PMOS contract broadening, tolerance, iteration budget, generic convergence, workflow or unrelated solver change. | `6efcf896383780cdb6e0265e68542ceb6ec0bf60` | **CLAIMED 2026-09-19.** bwlang confirmed its active DOS/toolchain lanes do not touch MNA/solver; implementation held until this remote-first row lands. |
