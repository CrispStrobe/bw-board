...
| --- | --- | --- | --- |
| strict source-analysis AC excitation/grid/regularization contract | `/root/zenodo_access_diagnosis` (Codex Sol), worktree `/mnt/volume1/code/wt/bw-board-ac-source-contract-sol` | `lane/ac-source-contract-sol` at exact base `2c92d52fcd6a893316aad3d51287b2dc8d0a09b6` | **CLAIM 2026-09-14.** Owns only `src/ac.js`, the `BoardImpl.runAc` option bridge, focused Board AC tests, and this row. Add validated exact frequency vectors, unit voltage/current-source phasor excitation, and explicit node regularization with the legacy interactive default preserved. Strict source analysis may select zero regularization; singular/floating systems must refuse rather than gain hidden conductance. Measured target is one exact Twin-T deck duplicated at ADI v4 rows 2651, 3542, 6576, 6596, 11159 and 13361 (39 AC real-component misses each, 234 total); no tolerance/profile waiver, CUI source-analysis edits, transient/DC equations, model substitution, oracle/comparator, or corpus artifact edits. |
