...
| --- | --- | --- | --- |
| explicit automatic-supply fallback policy | `/root/schematic_corpus_import` (Codex Sol), worktree `/tmp/bwb-rail-policy` | `lane/explicit-rail-fallback-sol` at exact base `cce98b4` | **CLAIM 2026-09-13; selective takeover, bwlang worktree/companion API untouched.** Replace blanket suppression of registered `_staticDrives` on an ideal `vcc` net with an explicit per-device-model opt-in containing only development-board automatic positive-supply fallback terminals (`5v`/`3v3`/`vbus`/`vsys`). Preserve physical battery/solar/regulator sources, ground drives and dynamic GPIO drives, including their real conflict currents; retain the valid one-Norton-per-drive correction from `df2eca2`. Correct the existing test's claim that all such circulating current is fictional, and prove a 9 V / 1 ohm source held at 5 V retains 4 A against ngspice, while the opted-in dev-board fallback remains suppressed and unpinned fallback still powers a load. Add ground/GPIO controls. Also clarify the already-conservative same-net ideal VCVS OP refusal wording without changing its policy. Envelope: `src/devices.js`, `src/devices/board-kinds.js`, `src/mna.js`, `src/board.js`, `test/ideal-rail-outranks-device-drive.test.mjs`, the focused OP test wording, and this row. No CUI/Lite/package/model-companion work. |
