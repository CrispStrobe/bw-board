...
| --- | --- | --- | --- |
| static OP same-net nonzero voltage-source guard | `/root/schematic_corpus_import` (Codex Sol) | `/tmp/bwb-op-ground-short`, `lane/op-ground-short-constraint`, exact base `b75caaa` | **CLAIM 2026-09-13.** Owns only `src/board.js`, `test/operating-point.test.mjs`, and this row. Before the public non-mutating `operatingPoint()` solve, reject a fixed ideal voltage source with nonzero DC value when its positive and negative terminals resolve to the same engine net. Preserve a zero-volt redundant same-net source using the same extra-resistor-node control. No `runAc`, transient/runtime, MNA allocation, other part kinds, CUI, pins, or deployment. |
