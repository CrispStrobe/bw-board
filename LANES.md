...
| --- | --- | --- | --- | --- | --- |
| strict explicit-NPN public DC operating-point domain | `/root` (Codex bwcx) | `/mnt/volume1/code/wt/bwb-bjt-op-domain`, branch `lane/bjt-operating-point-domain` | `src/board.js`, and only if measured convergence accuracy requires it, a public-OP-only nonlinear refinement option in `src/mna.js`; one focused NPN operating-point test, the existing operating-point scope-contract assertion, and this row. Admit only three connected terminals and explicit `model:'shockley'`, finite `is>0`, `beta>0`, optional finite `br>0`/`n>0`/`vaf>0`, plus inert source-model naming metadata; preserve the default/gallery knee path and all live state. Self-authored active/saturated/cutoff, signed terminal currents/KCL, exact ngspice conditions, strict refusals and caller-consequence mutations are required. Qualification then replays the exact 851-row historical ADI v4 NPN family at unchanged comparison tolerances. No PNP/MOS, CUI/parser/package, workflow, default interactive model, global tolerance, or unrelated solver work. | `2064764995455109ca56dced3939e036462dfaf4` | **CLAIM amended 2026-09-19.** Read-only measurement: 851/851 rows contain exactly one NPN and one `NPN(IS=1e-14 BF=100 VAF=100)` model; 160 unique normalized circuit bodies. The representative 6.18 µV collector gap was not convergence: ngspice 42's internal thermal constant differs slightly from the modern-CODATA conversion used by the private oracle. At the ngspice temperature that actually yields the engine's declared `VT=0.02585`, the same base/emitter/collector nodes agree within 0.02/0.01/1.9 nanovolts and source current within 0.2 picoamps; therefore no `src/mna.js` change is currently justified. Comparison tolerances will not be widened. |
