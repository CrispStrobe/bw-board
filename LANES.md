...
| --- | --- | --- | --- |
| bounded transient execution efficiency and deterministic work budgets | `/root/zenodo_access_diagnosis` (Codex Sol), worktree `/mnt/volume1/code/wt/bwb-waveform-efficiency-sol` | 2026-09-14, `lane/waveform-efficiency-sol`, exact base `d25376cb4150335fe3cf28a26e63994b6b96a03f` | **DONE.** Implementation `7bb750b83bda168863372a945fa9248d8ba1e5ab`; CI `34868399758` and Harris qualification `34868399744` succeeded (duplicate dispatches `34868703859`/`34868703799` also succeeded and add no evidence). A no-scope/no-state/no-reactive transient is now explicitly `algebraic-direct`: one independently qualified MNA solve per requested endpoint instead of adaptive integration between observations. Measured Si7li row5929 (28 series SINE sources + 1 resistor, `.tran 6`) completes its 101-point schedule as 100 attempts/100 solves/100 advances in 1.25 s including import/init, versus the prior timeout and 301 solves for one 1 ms advance. Reactive circuits and scope time grids retain the existing adaptive profile; current polarity, initialization, tolerances and budgets are unchanged. Historical row3525 is a real RC and remains an honest work-budget refusal rather than an approximation. |
