...
| --- | --- | --- | --- |
| explicit Shockley diode DC operating point | `/root/zenodo_access_diagnosis` (Codex Sol), worktree `/mnt/volume1/code/wt/bw-board-diode-op-sol` | `lane/diode-op-sol` at exact base `041d82d457c80e3dee45e6ef17e45dd4ba6a16c1` | **CLAIM 2026-09-13.** Extend only the public `BoardImpl.operatingPoint()` envelope to a two-terminal `diode` whose authored params explicitly select `model:'shockley'` with finite `is>0`, `n>0`, `rs>=0`; both terminals must be supplied and existing R/V anchoring remains required. Default/PWL, LED/zener/BV, unknown temperature/model physics and non-finite/defaulted fields refuse. Export one shared fixed thermal-voltage constant from `src/mna.js` for API metadata without changing equations. Preserve existing diode-current sign and live state. Self-authored arbitrary-parameter forward/reverse R-D-V ngspice proofs only; no vendor model bytes/values/results. Exact envelope: `src/board.js` OP preflight/metadata, `src/mna.js` thermal constant export, one focused test, bounded existing API docs, and this row. No CUI/Lite changes or solver-equation rewrite. |
