...
| --- | --- | --- | --- |
| grounded-positive independent voltage source MNA row | `/root/sol_lane_coordination` (Codex/Sol), root coordinates | `lane/ground-positive-vsource-sol` at exact base `10d6d18` | **CLAIM 2026-09-13.** Correct only independent-vsource extra-row admission when `pos` is the reference node and `neg` is live; the existing stamp already supports either side. Add a focused two-orientation anti-vacuity regression proving voltage polarity, signed source/load currents, and KCL. Envelope: `src/mna.js`, one focused test, this row. Then rerun the fixed five-case Si7li R/V/I manifest privately; no importer, parts-model, or broader solver changes. |
