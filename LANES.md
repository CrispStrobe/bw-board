...
| --- | --- | --- | --- |
| AVR code-breakpoint bound deduplication | sim2cx (Codex) | `converge/avr8js-code-bound-v2` from exact `52a46cc9e40f5507597105fc0d3f69035bc9da76` | **CLAIM 2026-09-13.** Derive one per-instance AVR program-memory maximum and refusal text, then use both for the run-to descriptor, code-breakpoint guard, and refusal. Envelope is `src/avr8js-debug.js`, `test/avr8js-debug.test.js`, and this row. Three isolated caller-consequence mutations cover descriptor, enforcement, and refusal drift. No Lite pin, other debugger target, workflow, or default change. |
