...
| --- | --- | --- | --- | --- | --- |
| strict explicit-NPN public DC operating-point domain | `/root` (Codex bwcx) | `/mnt/volume1/code/wt/bwb-bjt-op-domain`, branch `lane/bjt-operating-point-domain` | `src/board.js`, and only if measured convergence accuracy requires it, a public-OP-only nonlinear refinement option in `src/mna.js`; one focused NPN operating-point test plus this row. Admit only three connected terminals and explicit `model:'shockley'`, finite `is>0`, `beta>0`, optional finite `br>0`/`n>0`/`vaf>0`, plus inert source-model naming metadata; preserve the default/gallery knee path and all live state. Self-authored active/saturated/cutoff, signed terminal currents/KCL, exact ngspice conditions, strict refusals and caller-consequence mutations are required. Qualification then replays the exact 851-row historical ADI v4 NPN family at unchanged comparison tolerances. No PNP/MOS, CUI/parser/package, workflow, default interactive model, global tolerance, or unrelated solver work. | `2064764995455109ca56dced3939e036462dfaf4` | **CLAIM 2026-09-19.** Read-only measurement: 851/851 rows contain exactly one NPN and one `NPN(IS=1e-14 BF=100 VAF=100)` model; 160 unique normalized circuit bodies. A representative bypass measurement differs from precise ngspice by 0.18 µV at base, 6.18 µV at collector and 0.52 nA at the source, so solve accuracy is measured before any model change and comparison tolerances will not be widened. |
