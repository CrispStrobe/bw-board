...
| --- | --- | --- | --- |
| static native DC operating point API | `/root/schematic_corpus_import` (Codex Sol), worktree `/tmp/bwb-operating-point.vIxUHh` | `9eb334c`, 2026-09-13 | **CLAIM.** Add a public, observational `BoardImpl.operatingPoint()` and Boundary-B type for grounded static native R/C/V/I/GND/VCC only. Capacitors are DC-open; independent sources must be fixed DC without current limiting; nonlinear, registered/stateful, L/transformer, time-varying, and DC-floating networks refuse explicitly. Return independent node-voltage and normalized current-into-terminal maps plus convergence/conflict/analysis metadata. Reuse `solveMNA` without cap/transient history on shallow accepted-domain part copies; never advance, snapshot/restore, adopt, notify, or mutate live cache/time/history/state. Scope: `src/board.js`, `src/types.js`, one focused test; no MNA algorithm, runAc behavior, Lite/vendor, corpus/index, CUI, or dependency-pin edits. |
