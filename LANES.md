...
| --- | --- | --- | --- |
| ideal controlled-source DC operating point | `/root/schematic_corpus_import` (Codex Sol), worktree `/tmp/bwb-controlled-op` | `lane/controlled-op-sol` at exact base `8242e63` | **CLAIM 2026-09-13; explicit selective takeover while bwlang is offline.** First, fix only the VCVS MNA row allocator for grounded `outp` / live `outn`, with both polarities, live-node KCL and ngspice `.op` evidence. Then extend the existing non-mutating `BoardImpl.operatingPoint()` contract only to ideal finite-gain VCVS and ideal finite-`gm` VCCS: capacitors remain DC-open; VCVS output alone joins the DC-anchor graph; control pins and VCCS outputs do not; floating networks refuse; same-net VCVS output refuses; rail clamps, output resistance, current limits and non-finite/unknown parameters refuse. Public result currents remain positive into each named terminal, with VCCS normalized at the API boundary and no change to live `branchCurrent`. Envelope: `src/mna.js`, `src/board.js`, `src/types.js`, the exact public-OP paragraph in `docs/STANDARD-PARTS-PROGRAMME.md` (cleared with its prior owner), focused controlled-source/operating-point tests, and this row. Explicitly excludes bwlang's clean local `deviceCompanions` commit/worktree, the separate `_staticDrives` rail-policy concern, CUI/Lite/package files, and broader model changes. |
