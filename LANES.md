...
| --- | --- | --- | --- |
| BJT junction-exclusion sentinel repair | `/root/schematic_corpus_import` (Codex Sol) | `/tmp/bwb-bjt-exclusion-sentinel`, `lane/bjt-exclusion-sentinel`, exact base `42e974b` | **CLAIM 2026-09-13.** Owns `test/junction-knee-classes.test.mjs`, this row, and at most the stale non-executable `stampNPN` comment in `src/mna.js` after coordination with the active oracle-sweep owner. Repair the expired fixed-width/comment-sensitive source sentinel so it inspects exact executable stamp bodies. Preserve the real exclusion: BJT B-E stays piecewise; drive-derived Vce(sat) does not add a Shockley counterpart. No equation/model/runtime/package or Lite changes. |
