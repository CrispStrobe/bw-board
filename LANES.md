...
| --- | --- | --- | --- |
| exact ideal-inductor DC operating point | `/root/zenodo_access_diagnosis` (Codex Sol), worktree `/mnt/volume1/code/wt/bw-board-inductor-op-sol` | `lane/inductor-op-sol` at exact base `b8606c9efd488466ecbbc8cbdd812c993ff42082` | **CLAIM 2026-09-13.** Extend only non-mutating `BoardImpl.operatingPoint()` for a native two-terminal inductor with explicit finite `henrys > 0`. In the OP snapshot only, lower validated L to the existing exact 0 V ideal-source primitive on cloned parts/nets and map its signed current back to `a`/`b`; ordinary DC's 1 mΩ regularization and all transient BE/trapezoidal equations/state remain unchanged. L is an OP anchoring constraint. Refuse disconnected/self-shorted L, unknown IC/Rser/coupling/other params, and parallel L/0 V ideal-source constraints whose individual branch currents are indeterminate. Exact envelope: `src/board.js` OP validation/snapshot/helper, one new focused test, bounded `src/types.js` and existing OP programme docs, plus this row. Self-authored ±RL ngspice DC, KCL/current sign, nonzero stored-energy/time/control deep non-mutation, existing transient-energy regression, and refusal/nonconvergence proofs; no corpus/vendor payload, MOS work, CUI/Lite change, or `src/mna.js` equation edit. Candidate branch CI must be terminal green before guarded canonical fast-forward. |
