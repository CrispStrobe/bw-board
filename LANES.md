...
| --- | --- | --- | --- |
| exact seven-argument SPICE PULSE voltage waveform | schematic_corpus_import (Codex), worktree `/mnt/volume1/code/wt/bw-board-spice-pulse` | `lane/spice-pulse-vsource` at exact base `51c3c9154e0def15b1a0f6772f92ec942a133172` | **CLAIM 2026-09-13.** Add one distinct `wave: 'spice-pulse'` engine primitive for voltage sources with explicit finite `{v1,v2,td,tr,tf,pw,per}` values. Initial safe domain requires positive rise/fall/period, non-negative delay/width, and `tr+pw+tf <= per`; zero-edge SPICE default semantics, omitted arguments, expressions, overlap, and current-source waveforms remain explicit refusals. Preserve the established native frequency/duty `wave: 'pulse'` behavior and public operating-point refusal of every time-varying source. Envelope is new `src/source-waveforms.js`; only the `sourceVoltage` dispatch/import/docs in `src/mna.js`; only SPICE-PULSE corner alignment/import in `src/board.js`; new `test/spice-pulse-source.test.mjs`; and this row. Proof: analytic asymmetric/negative/repeated samples, actual Board RC transient, invalid/refusal and legacy controls, plus an independently sampled self-authored ngspice transient. No CUI/importer, Lite pin, or corpus-result edits. Corpus scale is contextual only: 1,549/1,748 exact-seven ASC occurrences are finite-literal candidates before timing constraints and other circuit blockers; this lane makes no whole-circuit or numerical-pass count claim. |
