...
| --- | --- | --- | --- | --- | --- |
| Level-1 NMOS exact cutoff with one-sided C1 threshold transition | `/root` (Codex bwcx) | `/mnt/volume1/code/wt/bwb-nmos-threshold-cutoff`, branch `lane/nmos-threshold-cutoff` | `src/mna.js`, `test/nmos-operating-point.test.mjs`, `test/mosfet-body-effect.test.mjs`, and this row only. The exact 1,670-row ADI-v5 model-default replay leaves 174 numerical failures; a representative nominal cutoff has VGS=VTO yet the current symmetric blend manufactures 156.25 nA (`K=0.1`, effective overdrive 1.25 mV), yielding 45.2 uV at source and 0.301 mV at drain while ngspice Level-1 and the hard square law are zero. Replace only the no-`ksubthres` threshold blend with a one-sided C1 transition that is exactly zero at/below threshold, joins the exact square law with value/slope continuity at the existing 5 mV boundary, and leaves declared VDMOS soft-plus unchanged. Prove cutoff zero/current/KCL, active-law continuity, the historical cross-coupled-latch convergence reason, and isolated zero-at-threshold/slope/VDMOS mutations; replay all 174 red rows downstream. No CUI/corpus/pin, PMOS contract broadening, tolerance, iteration budget, generic convergence, workflow or unrelated solver change. | `6efcf896383780cdb6e0265e68542ceb6ec0bf60` | **CLAIMED 2026-09-19; scope clarified before shared-test edit.** bwlang confirmed its active DOS/toolchain lanes do not touch MNA/solver. The existing smoothing test is included because its symmetric-band finite-difference oracle must move with the promised C1 boundary; no new production scope. |
| protected 286 restart, privilege and AT devices | Astra coordinator / up to two Sol agents, astra-x86-protection-at | `/mnt/volume1/code/wt/astra-x86-protection-at` and isolated worker trees | Restartable protected strings and common ISA; protected system registers, LDT/TSS and bounded privilege transitions; fuller opt-in AT keyboard/interrupt/RTC contracts. Owned guests, pinned PCjs, exact-source receipts and hosted qualification. Preserve production core throughput, upstream DOS/input work and other lanes. No FPGA, analog, UI/package pins or unproved full AT/386/application claim. | `69f052d16a0a17959234918e4539f8daaea5d861` | **CLAIMED 2026-09-19**; two Sol workers implement; Astra audits, steers and lands qualified combined milestones. |
