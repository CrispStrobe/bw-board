...
const samples=[],modes=['reference','compiled','native'];let expected;
for(let round=-1;round<rounds;round++)for(const mode of round%2?[...modes].reverse():modes) {
    const f=await createPhaseCircuitOracle({wasmBytes,Circuit:mode==='compiled'?CompiledDigitalCircuit:DigitalCircuit});
    ...
    const state=mode==='native'?f.kernel.inspect():captureWiredNetImage({enabled:true,circuit:f.circuit});
    const memory=f.refs.map((ref,i)=>{
        const m=mode==='native'?f.kernel.inspectMemory(i):{bytes:ref.state.mem,writes:ref.writes};assert.equal(m.writes,count);
...
const report={benchmark:'owned-latched-memory-components',accepted:true,capacityClaim:false,fullBoard:false,cpu:false,rounds,warmupRounds:1,count,
    ...
    notes:['Two SRAM banks/64 KiB, ideal controller/latch and actual nets only. No CPU, timers, DMA, interrupts or DOS workload.',
        ...
        'Native wrapper copies typed input and diagnostic output arrays at each begin/end boundary; not a native chunk runner.',
