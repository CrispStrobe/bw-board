...
const output=JSON.stringify({benchmark:'owned-wired-full-board',node:process.version,host:{platform:platform(),arch:arch(),cpu:cpus()[0]?.model,logicalCPUs:cpus().length},
...
if(process.env.HARRIS_BENCH_REPORT)writeFileSync(process.env.HARRIS_BENCH_REPORT,output+'\n',{flag:'wx'});
console.log(output);
