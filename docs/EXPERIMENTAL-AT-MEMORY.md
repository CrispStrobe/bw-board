...
The optional controller decodes ports 60h and 64h. Output-port operations D0
and D1 control A20 through bit 1; a bit-0-low write remains refused because
CPU reset pulses are not implemented. The controller now also provides a
bounded keyboard queue, command-byte access, interface enable/disable and
self-test replies. See [the AT device profile](AT-DEVICE-PROFILE.md) for the
precise keyboard, cascaded PIC and RTC contract. A configured device window
covering 60h or 64h is rejected; port 61h remains available for the speaker/PPI.
Port 92h fast A20 is not part of this original-AT profile.
...
default CPU checkpoint also carries the live gate, output port, pending controller command,
and typed output queue and validates all of them before mutation.
