...
| MIT xv6 x86 (`eeb7b415`) | experimental 386 AT | Source builds reproducibly with host 32-bit GCC; the IBM 5170 Rev1 ROM now runs through POST memory status codes but remains in its 0x31/0x32 verification loop for 10M instructions with zero ATA commands, so no OS acceptance yet |
