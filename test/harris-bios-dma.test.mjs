...
registerBusMemory();
for(const [netBackend,memoryScheduling] of [['reference',false],['compiled',false],['compiled',true]])test(`${netBackend}/${memoryScheduling}: owned guest invokes unmodified BIOS INT 13h and reads a sector through DMA/IRQ6`,()=>{
...
HLT`,0x6000);
    ...
    const board=createHarrisMemoryBoard({enabled:true,rom,romLowAlias:true,intrEnabled:true,ioEnabled:true,holdEnabled:true,netBackend,memoryScheduling,
