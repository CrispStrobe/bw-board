...
import {MASTER_REVISION,LEGACY_WORK_COUNTERS,NATIVE_STAGES,assertSemantic,assertStageReceipt,classifyProfile,rotatedOrder,summarize}
...
function semantic(){return {stateHash:'x',periods:10,retired:3,writes:[1,1],physicalClock:77,chunks:1,yields:0,
...
function stages(){return {native:{memoryMappingCalls:5,memoryMappingVisits:3200,memoryGatherCalls:6,memoryGatherPinRecords:672,memoryPreviewCalls:6,
    ...
    js:{wasmBusInspectEntries:46,wasmBusSubmitEntries:9,wasmBusRunEntries:10,completionObjects:10,nativeReceiptBytesRead:360}};}
...
test('stage receipt reconciles exact loop dimensions and receipt crossings',()=>{
    ...
    for(const [section,name] of [['native','memoryMappingVisits'],['native','memoryCommitStateWordCopies'],['js','nativeReceiptBytesRead']]){
