...
test('rotating order and summaries are deterministic',()=>{assert.deepEqual([1,2,3,4].map(rotatedOrder),[['master','diagnosticOff','nativeCounter'],['diagnosticOff','nativeCounter','master'],['nativeCounter','master','diagnosticOff'],['master','diagnosticOff','nativeCounter']]);assert.equal(summarize([1,2,3]).median,2);});
...
test('workflow pins exact control, builds off/on separately, and rejects weak attribution',()=>{
    ...
    assert.match(workflow,/candidateOffToMaster>=\.98&&r\.ratios\.candidateOffToMaster<=1\.02/);assert.match(workflow,/nativeCounterToOff>=\.98&&r\.ratios\.nativeCounterToOff<=1\.02/);assert.doesNotMatch(workflow,/r\.ratios\.combinedCounterTo(?:Off|Native)/);assert.match(workflow,/untimedCombinedReceipt\.semantic,r\.expected/);assert.match(workflow,/\["activeMS","wallMS","activePeriodsPerSecond"\]/);assert.match(workflow,/stageProfileNames\],\[true,false,false\]/);assert.match(workflow,/stageProfileNames\],\[false,false,true\]/);
...
test('timed native neutrality and untimed JavaScript counts stay disjoint',()=>{const runner=source('scripts/measure-harris-native-stage-attribution.mjs');
    ...
    assert.doesNotMatch(runner,/combinedCounterToOff|combinedCounterToNative/);
    assert.match(runner,/contributes no timing sample, ratio, capacity, neutrality, cost, or speed claim/);
});
