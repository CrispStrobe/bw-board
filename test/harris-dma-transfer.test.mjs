...
function fixture({count=4,address=0x501,mode=0x46,roundTrip=false,netBackend='reference',memoryScheduling=false,memoryWriteJournal=false,decoderSpecialization=false,deviceScheduling=false,packedBus=false,editWires=w=>w}={}){
    ...
    const board=createHarrisMemoryBoard({enabled:true,rom,romLowAlias:true,holdEnabled:true,intrEnabled:true,ioEnabled:true,
        interruptDevice:new Harris8259Adapter({enabled:true}),fdcDevice:fdc,dmaDevice:dma,netBackend,memoryScheduling,memoryWriteJournal,decoderSpecialization,deviceScheduling,packedBus,editWires});
...
for(const [memoryScheduling,memoryWriteJournal,decoderSpecialization=false,deviceScheduling=false,packedBus=false] of [[false,false],[true,false],[true,true],[true,false,true],[true,false,false,true],[true,false,false,true,true]])test(`compiled connectivity matches reference DMA ownership, data and guest outcome each period (scheduled=${memoryScheduling},journal=${memoryWriteJournal},specialized=${decoderSpecialization},events=${deviceScheduling},packed=${packedBus})`,()=>{
    for(const options of [{},{mode:0x4a,roundTrip:true},{mode:0x42}]) {
        const ref=fixture(options),fast=fixture({...options,netBackend:'compiled',memoryScheduling,memoryWriteJournal,decoderSpecialization,deviceScheduling,packedBus});
        let clocks=0;
