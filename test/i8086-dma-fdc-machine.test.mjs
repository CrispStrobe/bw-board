...
/** The XT arrangement: DMA at 00h, its page latch at 80h, the FDC at 3F0h. */
const XT_DISK = Object.freeze({
    clockHz: 4_772_727,
    ...
    chips: [
        ...
        { kind: 'fdc', name: 'fdc1', at: 0x3f0, irq: 6 },
    ],
});

test('a machine config can name a dma and an fdc at all', () => {
    const m = new I8086Machine(XT_DISK);
...
test('the page bytes survive a snapshot, and are not stored twice', () => {
    ...
    const m2 = new I8086Machine(XT_DISK);
    m2.loadState(snap);
