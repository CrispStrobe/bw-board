...
async function rawBus(){
    ...
    const p={context:alloc(35*4),offsets:alloc(100*4),ids:alloc(99*4),drivers:alloc(99),live:alloc(99),conflicts:alloc(99),
        ops:alloc(4),staged:alloc(99),published:alloc(99),publishedConflicts:alloc(99),dependencyOffsets:alloc(4),
        dependencies:alloc(4),previous:alloc(99),changed:alloc(99),memory:alloc(32768),states:alloc(9*4),memoryStaged:alloc(9*4),
    ...
    put(p.context,[99,99,p.offsets,p.ids,p.drivers,p.live,p.conflicts,0,p.ops,p.staged,p.published,p.publishedConflicts,8,
    ...
    put(p.controllerIds,Array.from({length:7},(_,i)=>58+i));
    put(p.latchInputNets,[58,...Array.from({length:24},(_,i)=>i),40,44]);
    put(p.latchIds,Array.from({length:26},(_,i)=>65+i));
    ...
    const execution=()=>({drivers:Array.from(new Uint8Array(e.memory.buffer,p.drivers,99)),
...
test('bus admission refuses each final map entry, bound and live value with its existing receipt',native,async()=>{
    const cases=[
        ...
        ['input',23,k=>k.v.setUint32(k.p.busInputNets+23*4,99,true)],
        ['output',47,k=>k.v.setUint32(k.p.busOutputIds+47*4,99,true)],
        ['external',9,k=>k.v.setUint32(k.p.busExternalIds+9*4,99,true)],
...
test('post-admission arena map edits cannot redirect any of the three consumers',native,async()=>{
    ...
    }
    {
        ...
        assert.equal(drivers[46],0,'captured map leaves canonical output 46 unchanged');
        assert.equal(drivers[47],1,'captured map publishes final output to its admitted driver');
    }
});

test('every bus, graph and memory re-admission attempt revokes the preceding bus grant',native,async()=>{
    {
        const k=await admitted();k.v.setUint32(k.p.busOutputIds+47*4,99,true);
...
test('raw validator accepts count 128 and refuses 129 before a writer',native,async()=>{
    {
        ...
        assert.notEqual(k.e.begin_bus_memory_clock(k.p.busContext,k.p.fault),8,'exact raw count boundary passes mapping validation');
        assert.equal(k.execution().producer[1],128,'all 128 raw external entries reach publication');
    }
    {
